...
const VERIFY_TEXT = "Verify the Casino contract with this command: \n npx hardhat verify --network "
const NETWORK_NAME = "maticmum"

const BUY_AMOUNT = 100;
const STAKE_AMOUNT = 4;
...
async function main() {
  ...
  console.log(
    `${VERIFY_TEXT} ${NETWORK_NAME} ${token.address} `
  );

  console.log(
    `Verify the NFT contract with this command: \n
    npx hardhat verify --network maticmum ${nft.address} 
    `
  );
  console.log(
    `Verify the Casino contract with this command: \n
    npx hardhat verify --network maticmum ${
      contract.address
  ...
  // const unstakingTx = await contract.unstake(ethers.utils.parseEther("1"));
